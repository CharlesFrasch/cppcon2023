...
/// Thread-safe, try-lock mutex-based FIFO
template<typename T, typename Alloc = std::allocator<T>>
class TryLock : private Alloc
{
public:
    using value_type = T;
    ...
    explicit TryLock(size_type capacity, Alloc const& alloc = Alloc{})
        : Alloc{alloc}
        , capacity_{capacity}
    ...
    auto push(T const& value) {
        if (std::unique_lock<std::mutex> lock(mutex_, std::try_to_lock); lock.owns_lock()) {
